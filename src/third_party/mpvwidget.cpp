    ...
    : QOpenGLWidget(parent, f)
{
    ...
    mpv::qt::set_option_variant(mpv, "gpu-hwdec-interop", "auto");
...
void MpvWidget::tryInitGL()
{
    if (mpv_gl || m_glInitFailed)
        return;
    ...
    if (mpv_render_context_create(&mpv_gl, mpv, params) < 0) {
        qWarning() << "[VideoWallpaper] failed to initialize mpv GL context - skipping screen";
        m_glInitFailed = true;
        return;
    }
...
{
    tryInitGL(); // no throw - shell survives if this fails
}

void MpvWidget::paintGL()
{
    if (!mpv_gl) {
        tryInitGL(); // retry - handles portrait/timing edge cases
        if (!mpv_gl)
            return;
    }
